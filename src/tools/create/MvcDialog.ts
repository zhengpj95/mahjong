...
export class MmvDialog extends IEditor.Dialog {
  async create() {
    this.contentPane = await gui.UIPackage.createWidget(
      "editorResources/MvcDialog.widget",
    );
  }

  onShown() {
    this.title = "MVC -- 构建逻辑模块";
    const textInput = this.contentPane
      .getChild("labModule")
      .getChild("textInput") as TextInput;
    textInput.text = "";
  ...
  private createModule(moduleName: string): void {
    ...
    if (dirList.includes(moduleName)) {
      console.error(
        `❌ MVC 创建模块：src/modules/ 已存在 ${moduleName} ，不可重复创建`,
      );
      return;
    }

    dirList.length = 0;
    const sceneDirs = fs.readdirSync(
    ...
    if (dirList.includes(moduleName)) {
      console.error(
        `❌ MVC 创建模块：assets/scene/ 已存在 ${moduleName} ，不可重复创建`,
      );
      return;
    }

    startCreate(moduleName)
      .then((value) => {
