...
export class SplitAtlasDialogNew extends IEditor.Dialog {
  ...
  async create() {
    this.contentPane = await gui.UIPackage.createWidget("editorResources/SplitAtlas.widget");
    this.title = "图集分割工具";
  }

  protected onShown() {
    this.contentPane.getChild("btnConfirm").on("click", this.btnConfirmFunc, this);
    this.contentPane.getChild("btnCancel").on("click", () => {
      console.log(11111, new Date().toLocaleTimeString());
      this.hide();
    }, this);
    this._comboBox = <ComboBox>this.contentPane.getChild("boxSplitType").getChild("ComboBox");
    ...
    this._boxInput.getChild("IconButton").on("click", () => {
    ...
    this._boxOutput.getChild("IconButton").on("click", () => {
      Editor.showOpenDialog({ title: "select file", properties: ["openDirectory"] }).then(value => {
