...
export class GenUIDts {
  public static async generateDts(dir: string): Promise<void> {
    ...
    for (const file of files) {
      ...
      if (DEBUG) {
        console.log(
          `▶️ start to generate ls file: `,
          file.replace(toolsObj.ProjectRoot, ""),
        );
      }
  ...
  public static async parseChild(
    child: IChildNode,
    parent: {
      [key: string]: NodeInfo;
    },
    ...
    if (child?._$prefab) {
      ...
      if (child?._$child?.length) {
        ...
        if (prefabInfo.node && Object.keys(overrideObj)) {
          for (const k in prefabInfo.node) {
  ...
  public static async createPrefabNode(
    uuid: string,
    name: string,
  ): Promise<NodeInfo | undefined> {
    const prefabPath = await FsUtils.getPrefabByUUID(uuid);
  ...
  public static async createComponentNode(
    scriptPath: string,
  ): Promise<string[]> {
    if (!scriptPath) return [];
    const basename = path.basename(scriptPath);
    const compName = basename.replace(".ts", "");
    const prefabUIContent = await fs.readFile(
      path.join(toolsObj.ProjectRoot, "src/tools", "genUI/PrefabUI.json"),
      { encoding: "utf-8" },
    );
    return JSON.parse(prefabUIContent)[compName] ?? [];
  }

  public static async writeDtsFile(
    fileContent: string,
    basename: string,
  ): Promise<void> {
    ...
    try {
      ...
      console.log(
        `✅ write ui .d.ts success: ${filePath.replace(toolsObj.ProjectRoot, "")}`,
      );
    } catch (err) {
      console.log(
        `❌ write ui .d.ts error: ${filePath.replace(toolsObj.ProjectRoot, "")}`,
        err,
      );
    }
  }
